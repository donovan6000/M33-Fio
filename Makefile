...
ifeq ($(TARGET_PLATFORM), LINUX32)
	...
	CC = g++
	CFLAGS = -fPIC -m32 -static-libgcc -Ofast -Wl,-soname,$(PROG)$(VER)
endif

ifeq ($(TARGET_PLATFORM), LINUX64)
	PROG = $(LIBRARY_NAME)_x86-64.so
	CC = g++
	CFLAGS = -fPIC -m64 -static-libgcc -Ofast -Wl,-soname,$(PROG)$(VER)
endif

ifeq ($(TARGET_PLATFORM), WINDOWS32)
	PROG = $(LIBRARY_NAME)_i386.dll
	CC = i686-w64-mingw32-g++
	CFLAGS = -DM_PI_2=1.57079632679489661923 -static-libgcc -Ofast -Wl,-soname,$(PROG)$(VER)
endif
...
ifeq ($(TARGET_PLATFORM), PI)
	...
	CFLAGS = -fPIC -mcpu=arm1176jzf-s -mfpu=vfp -mfloat-abi=hard -static-libgcc -Ofast -Wl,-soname,$(PROG)$(VER)
endif

ifeq ($(TARGET_PLATFORM), PI2)
	PROG = $(LIBRARY_NAME)_arm_cortex-a7.so
	CC = ~/tools/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian-x64/bin/arm-linux-gnueabihf-g++
	CFLAGS = -fPIC -mcpu=cortex-a7 -mfpu=neon-vfpv4 -mfloat-abi=hard -funsafe-math-optimizations -static-libgcc -Ofast -Wl,-soname,$(PROG)$(VER)
endif

ifeq ($(TARGET_PLATFORM), ARM7)
	PROG = $(LIBRARY_NAME)_arm7.so
	CC = ~/tools/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian-x64/bin/arm-linux-gnueabihf-g++
	CFLAGS = -fPIC -mcpu=generic-armv7-a -mfpu=vfp -mfloat-abi=hard -static-libgcc -Ofast -Wl,-soname,$(PROG)$(VER)
endif
...
CFLAGS += -Wall -std=c++11 -static-libstdc++ -fvisibility=hidden -shared
